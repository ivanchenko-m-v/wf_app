...
/*
	...
	от обратного id_portion = id_history существующей доли
*/
...
/*
	циклическа€ выборка цепочек долей - 
	от обратного id_portion = id_history первой доли в Ѕƒ
*/
	WITH portion_followers( 
							id_portion,
							id_history, 
	...
	(
			SELECT	
					...
					A.[id_history_was] AS [id_history_was]
				FROM [limits_2009].[dbo].[portion_history] A 
				WHERE (A.[id_history_was] IS NULL)OR(A.[id_history_was]=0)
		UNION ALL --последователи
			SELECT	
					C.[id_portion] AS [id_portion],
					B.[id_history] AS [id_history],
					B.[id_history_was] AS [id_history_was]
			FROM [limits_2009].[dbo].[portion_history] B 
				JOIN portion_followers C ON B.[id_history_was]=C.[id_history]
	)
	SELECT 
		    FA.id_portion
		   ,FA.id_history 
...
FROM 
 ...
 ( -- BS
	...
	-- RIGHT JOIN 
	 ...
	 ORDER BY  VW.id_basin, VW.id_regime, VW.id_fish, VW.id_region, VW.id_subject, FA.id_portion, FA.id_history;
/*
...
/*
	2017-07-20
	объединение цепочек долей и истории передачи долей
	id_portion=id_history первой доли
*/
	WITH portion_followers( 
							id_portion,
							id_history, 
