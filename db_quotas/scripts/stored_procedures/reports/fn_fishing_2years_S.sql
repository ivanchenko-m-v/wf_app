...
GO
*/
CREATE FUNCTION [dbo].[fn_fishing_2years_S]
(
	@nYear1 int, 
	@nYear2 int, 
	@nPercent int,
...
BEGIN
	...
	INSERT @tb_ret
		SELECT	 LM1.[id_basin], BS.[basin], LM1.[id_subject], SB.[subject],
				 ...
				 ISNULL(LM2.[catch_volume_stat], 0) AS catch_stat_2,
				 (ISNULL(LM2.[catch_volume_stat], 0) / LM2.[limits_volume])*100 AS percent_2  
				 FROM 
				 ...
				 ( --INNER JOIN [limits_2009c].[dbo].[basin] BS ON LM1.[id_basin]=BS.[id_basin])
					 (
						 ( SELECT	[id_basin], [id_subject], [id_declarant], [id_regime],
									...
									[portion], [limits_volume], [catch_volume_stat] 
